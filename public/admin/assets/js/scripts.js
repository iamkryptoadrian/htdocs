!(function (NioApp, $) {
    ...
    NioApp.CurrentLink = function() {
        var _link = '.nk-menu-link, .menu-link, .nav-link',
            ...
            _locationPathname = window.location.pathname,
            _locationOrigin = window.location.origin;

        $(_link).each(function() {
            var self = $(this),
                _self_link = self.attr('href'),
                _self_pathname = new URL(_self_link, _locationOrigin).pathname;

            if (_locationPathname === _self_pathname) {
                self.closest("li").addClass('active current-page').parents().closest("li").addClass("active current-page");
                self.closest("li").children('.nk-menu-sub').
                css('display', 'block');
                self.parents().closest("li").children('.nk-menu-sub').css('display', 'block');
                if (self.closest('.nk-menu-sub').length === 0) { // it's a top-level link, not a sub-menu link
