...
const Calendar = (function() {
  ...
  const startYear = currentDate.getFullYear(); // This will set the year to the current year
  const startMonth = currentDate.getMonth() + 1; // getMonth returns 0-11, so add 1 to make it 1-12 for the current month
  const monthRange = 12;
  ...
  const isDatePassed = (day, month, year) => {
  ...
  const getAllDaysInMonth = (month, year) => {
    let initialMonthArray = Array.from(
      ...
      (_, i) => {
        const dayNum = i + 1;
        const passed = isDatePassed(dayNum, month, year);
        return {
          weekDay: weekDaysOrder[new Date(year, month - 1, dayNum).getDay()],
          dayNum: dayNum,
          isPassed: passed, // Indicate whether the date has passed
        }
      }
    );
                ...
                <div class="swiper-slide">
                  ...
                  <div class="elCalendar__month">
                    ...
                    <div class="elCalendar__body">
                      ...
                      <div
                        data-index="${globalIndexUp()}" data-week="${el.weekDay}" data-month="${month.monthName.slice(0, 3)}"
                        class="elCalendar__sell${el.isPassed ? ' -dark cant_select' : ''}"
                      >
                        <span class="js-date">
                          ${el.dayNum}
                        </span>
...
function selectControl() {
  ...
  // No need to check if targets is not there because querySelectorAll always returns a NodeList
  // Even if empty, the forEach will just not iterate
  ...
  targets.forEach(el => {
    ...
    counters.forEach((counter, index) => {
      // Ensure the element exists before trying to access innerHTML
      const titleElement = counter.querySelector('.js-search-title');
      const title = titleElement ? titleElement.innerHTML : '';

      const counterTarget = counter.querySelector('.js-search-counter');
      if (!counterTarget) return;

      const numberElement = counterTarget.querySelector('.js-number');
      const removeButton = counterTarget.querySelector('.js-remove');
      const addButton = counterTarget.querySelector('.js-add');

      // Initialize array with title and value
      arr.push({ title: title, value: 0 });

      if (removeButton && numberElement) {
        removeButton.addEventListener('click', () => {
          const currentValue = parseInt(numberElement.innerHTML);
          if (currentValue > 0) {
            const newValue = currentValue - 1;
            numberElement.innerHTML = newValue;
            arr[index].value = newValue;

            updateChosenText(arr, chosen);
          }
        });
      }

      if (addButton && numberElement) {
        addButton.addEventListener('click', () => {
          const newValue = parseInt(numberElement.innerHTML) + 1;
          numberElement.innerHTML = newValue;
          arr[index].value = newValue;
...
function updateChosenText(arr, chosenElement) {
  const nonZeroValues = arr.filter(el => el.value > 0)
                           .map(el => ` ${el.title} ${el.value}`);
  chosenElement.innerHTML = nonZeroValues.join('');
}
